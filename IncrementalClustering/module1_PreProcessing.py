...
def strip_nonascii(tokenized):
    # print tokenized
     a= []
     ff=open(os.path.join(path , 'Crawled Articles' , 'temp1.txt'),'a')
...
def createWordSet():
    ...
    for filename in glob.glob(os.path.join(path , 'Crawled Articles' , newsPaperName , '*.txt')):
        f=open(filename, 'r')
        print 'Preprocessing file ', fileID, ' from ', newsPaperName
        fileID = fileID + 1
        ...
        ascii_f=open(os.path.join(path , 'Crawled Articles' , 'temp1.txt'),'r')
        
 
#lemmatization code starts here..
        lem_f=open(os.path.join(path , 'Crawled Articles' , 'temp3.txt'),'w')
        for sent in ascii_f:
...
#lemmatization code ends here....        
        ...
        ip=open(os.path.join(path , 'Crawled Articles' , 'temp3.txt'),'r')
        op=open(os.path.join(path , 'Crawled Articles' , 'temp2.txt'),'w')
        #sys.stdout = ff
        for sent in ip:
        ...
        ip.close()
        f=open(filename, 'w')
        fp = open(os.path.join(path , 'Crawled Articles' , 'temp2.txt'),'r')
        for line in fp:
            line = line.rstrip('\n')
        ...
        f.close()
        fp.close()
        fpo = open(os.path.join(path , 'Crawled Articles' , 'temp1.txt'),'w')
        fpo.close()
        fp1 = open(os.path.join(path , 'Crawled Articles' , 'temp2.txt'),'w')
        fp1.close() 
